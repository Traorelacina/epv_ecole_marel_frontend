...
import {
  ...
  GraduationCap, Trophy, Target, Sparkles, ChevronRight,
} from 'lucide-react'
...
/* ── Hero simplifié avec fond unique + cercle rotatif ── */
      ...
      <div className="wrap" style={{ position: 'relative', zIndex: 10, paddingTop: '80px', paddingBottom: '80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }} className="grid-cols-1 lg:grid-cols-2">
          ...
          <div>
            ...
            <h1 style={{ ...fadeUp(200), fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(2.4rem,5vw,3.8rem)', color: '#ffffff', lineHeight: 1.1, marginBottom: '10px', letterSpacing: '-1px' }}>EPV MAREL,</h1>
            <h1 style={{ ...fadeUp(340), fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(2.4rem,5vw,3.8rem)', color: '#8DC31E', lineHeight: 1.1, marginBottom: '10px', letterSpacing: '-1px' }}>une institution</h1>
            <h1 style={{ ...fadeUp(460), fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(2.4rem,5vw,3.8rem)', color: '#ffffff', lineHeight: 1.1, marginBottom: '28px', letterSpacing: '-1px' }}>d'excellence</h1>
          ...
          <div className="hidden lg:flex" style={{ justifyContent: 'center', alignItems: 'center', position: 'relative', opacity: loaded ? 1 : 0, animation: loaded ? 'scaleIn .9s cubic-bezier(.22,.68,0,1.2) 300ms both' : 'none' }}>
            ...
            <div style={{ width: '420px', height: '420px', borderRadius: '50%', background: 'rgba(141,195,30,0.10)', border: '1.5px solid rgba(141,195,30,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
              ...
              <div className="badge-float-b" style={{ position: 'absolute', bottom: '40px', left: '-20px', background: '#8DC31E', borderRadius: '16px', padding: '12px 18px', boxShadow: '0 8px 32px rgba(141,195,30,0.4)', display: 'flex', alignItems: 'center', gap: '10px', zIndex: 3 }}>
                ...
                <div><p style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: '14px', color: '#fff', lineHeight: 1 }}>Depuis 2000</p><p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.7)', marginTop: '2px' }}>Excellence</p></div>
              </div>
            </div>
          </div>
        </div>
...
export default function PresentationPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Présentation — EPV MAREL | Une institution d'excellence à Abidjan</title>
      ...
      {/* Histoire — sans mini-stats ni bouton "notre offre pédagogique" */}
      <section className="section" style={{ backgroundColor: '#F9FAFB' }}>
        <div className="wrap">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '72px', alignItems: 'center' }} className="grid-cols-1 lg:grid-cols-2" ref={ref1}>
            <div className={iv1 ? 'anim-slide-l' : ''}>
              <div style={{ borderRadius: '28px', overflow: 'hidden', boxShadow: '0 25px 50px rgba(45,106,31,0.2)', aspectRatio: '4/3', position: 'relative' }}>
                <img src={ecoleImage} alt="EPV MAREL - Vue de l'école" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '100px', background: 'linear-gradient(to top, rgba(0,0,0,0.4), transparent)' }} />
              </div>
            </div>
      ...
      <section className="section-sm" style={{ backgroundColor: '#ffffff' }}>
        <div className="wrap">
          <div style={{ borderRadius: '28px', background: 'linear-gradient(135deg,#2D6A1F 0%,#1a4010 100%)', padding: 'clamp(40px,6vw,60px)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'relative', zIndex: 2 }}>
              <h2 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(1.8rem,4vw,2.5rem)', color: '#ffffff', marginBottom: '16px' }}>Prêt à rejoindre la famille MAREL ?</h2>
              ...
              <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
                <Link to="/contacts" className="btn-white" style={{ fontSize: '15px', padding: '14px 32px' }}>Demander une inscription <ArrowRight size={16} /></Link>
              </div>
            </div>
          </div>
        </div>
