// ── InfosParentsPage (ex-InfosPratiquesPage) ─────────────────────────────────
...
import {
  ChevronRight, Clock, Calendar, FileText, MapPin,
  CheckCircle, AlertCircle, BookOpen, GraduationCap,
  Bell, School, ArrowRight, Download, Info, Shirt,
  ...
  return (
    <section style={{ position: 'relative', minHeight: '70vh', background: 'linear-gradient(155deg,#0f2a07 0%,#1a4010 30%,#2D6A1F 65%,#5a9c22 85%,#8DC31E 100%)', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }} className="grid-cols-1 lg:grid-cols-2">
          <div>
            <div style={{ ...fadeUp(100), display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
              ...
              <span style={{ color: '#8DC31E', fontSize: '13px', fontWeight: 600 }}>Informations Parents</span>
            </div>
            <div style={{ ...fadeUp(150), display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 18px', borderRadius: '50px', background: 'rgba(141,195,30,0.18)', border: '1px solid rgba(141,195,30,0.35)', marginBottom: '28px' }}>
              ...
              <span style={{ color: '#B5D95A', fontSize: '12.5px', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Espace parents</span>
            </div>
            <h1 style={{ ...fadeUp(220), fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(2.4rem,5vw,3.8rem)', color: '#fff', lineHeight: 1.1, marginBottom: '10px', letterSpacing: '-1px' }}>Tout savoir</h1>
            <h1 style={{ ...fadeUp(340), fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(2.4rem,5vw,3.8rem)', color: '#8DC31E', lineHeight: 1.1, marginBottom: '10px', letterSpacing: '-1px' }}>pour la scolarité</h1>
            <h1 style={{ ...fadeUp(460), fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 'clamp(2.4rem,5vw,3.8rem)', color: '#fff', lineHeight: 1.1, marginBottom: '28px', letterSpacing: '-1px' }}>de votre enfant</h1>
            <p style={{ ...fadeUp(580), color: 'rgba(255,255,255,0.75)', fontSize: '17px', maxWidth: '500px', lineHeight: 1.8, marginBottom: '40px' }}>
              Horaires, inscriptions, cantine, transport, uniforme et calendrier scolaire — toutes les informations pratiques.
            </p>
            <div style={{ ...fadeUp(700), display: 'flex', flexWrap: 'wrap', gap: '14px' }}>
              <Link to="/contacts" className="btn-red" style={{ fontSize: '15px', padding: '14px 28px' }}>Contacter l'école <ArrowRight size={17} /></Link>
            </div>
            <div style={{ ...fadeUp(820), display: 'flex', gap: '32px', marginTop: '52px', flexWrap: 'wrap' }}>
              {[{ val: '7h30', label: 'Ouverture' }, { val: '16h30', label: 'Fermeture' }, { val: '5j', label: 'Lun — Ven' }].map((s, i) => (
          ...
          <div className="hidden lg:flex" style={{ justifyContent: 'center', alignItems: 'center', position: 'relative', opacity: loaded ? 1 : 0, animation: loaded ? 'scaleIn .9s cubic-bezier(.22,.68,0,1.2) 300ms both' : 'none' }}>
            ...
            <div style={{ width: '380px', height: '380px', borderRadius: '50%', background: 'rgba(141,195,30,0.10)', border: '1.5px solid rgba(141,195,30,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
              <div style={{ position: 'absolute', width: '300px', height: '300px', borderRadius: '50%', border: '2px solid transparent', borderTopColor: '#8DC31E', borderBottomColor: '#D4191A', opacity: 0.6 }} className="ring-spin-cw" />
              <div style={{ width: '260px', height: '260px', borderRadius: '20px', overflow: 'hidden', border: '4px solid #fff', position: 'relative', zIndex: 2, boxShadow: '0 0 0 8px rgba(141,195,30,0.1),0 20px 40px rgba(0,0,0,0.3)' }}>
                <img src={infosImage} alt="Infos pratiques EPV MAREL" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
              <div className="badge-float-a" style={{ position: 'absolute', top: '-20px', right: '-10px', background: '#D4191A', borderRadius: '20px', padding: '12px 18px', boxShadow: '0 8px 32px rgba(212,25,26,0.3)', display: 'flex', alignItems: 'center', gap: '10px', zIndex: 3 }}>
                ...
                <div><p style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: '13px', color: '#fff', lineHeight: 1 }}>7h30 - 16h30</p><p style={{ fontSize: '9px', color: 'rgba(255,255,255,0.8)', marginTop: '2px' }}>Lundi au Vendredi</p></div>
              </div>
              <div className="badge-float-b" style={{ position: 'absolute', bottom: '-15px', left: '-15px', background: '#2D6A1F', borderRadius: '20px', padding: '10px 16px', boxShadow: '0 8px 32px rgba(45,106,31,0.4)', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 3 }}>
                ...
                <div><p style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: '12px', color: '#fff', lineHeight: 1 }}>Études surveillées</p><p style={{ fontSize: '9px', color: 'rgba(255,255,255,0.7)', marginTop: '2px' }}>16h00 - 17h00</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
        <svg viewBox="0 0 1440 80" fill="none"><path d="M0 80L1440 80L1440 30C1200 80 960 0 720 40C480 80 240 20 0 30L0 80Z" fill="#F9FAFB" /></svg>
      </div>
    </section>
  )
}
...
const TABS = [
  ...
  { key: 'cantine',     label: 'Cantine',             icon: Utensils },
  { key: 'transport',   label: 'Transport',           icon: Bus },
  { key: 'uniforme',    label: 'Uniforme',            icon: Shirt },
  { key: 'calendrier',  label: 'Calendrier scolaire', icon: Calendar },
  { key: 'reglement',   label: 'Règlement intérieur', icon: ScrollText },
]
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '40px' }}>
      {TABS.map(t => {
        ...
        return (
          <button key={t.key} onClick={() => onChange(t.key)} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 16px', borderRadius: '50px', border: 'none', cursor: 'pointer', fontFamily: "'Sora',sans-serif", fontSize: '13px', fontWeight: 600, transition: 'all 0.22s', backgroundColor: isActive ? '#2D6A1F' : '#ffffff', color: isActive ? '#ffffff' : '#374151', boxShadow: isActive ? '0 4px 16px rgba(45,106,31,0.28)' : '0 2px 8px rgba(0,0,0,0.07)', transform: isActive ? 'translateY(-1px)' : 'none' }}>
...
function HorairesTab({ horaires }) {
  ...
  return (
    <div ref={ref}>
      <div className={inView ? 'anim-fade-up' : ''} style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid #F3F4F6', boxShadow: '0 4px 24px rgba(45,106,31,0.08)' }}>
        <div style={{ background: 'linear-gradient(135deg,#2D6A1F,#5a9c22)', padding: '14px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '16px' }}>
          {['Niveau', 'Période', 'Horaire', 'Jours'].map(h => <p key={h} style={{ color: 'rgba(255,255,255,0.85)', fontSize: '11.5px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</p>)}
        </div>
        {rows.map((h, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '16px', padding: '16px 24px', backgroundColor: i % 2 === 0 ? '#ffffff' : '#F9FAFB', borderTop: '1px solid #F3F4F6', transition: 'background 0.2s' }}
            ...
            <p style={{ fontFamily: 'monospace', color: '#2D6A1F', fontWeight: 700, fontSize: '14.5px' }}>{(h.heure_debut || '').slice(0, 5)} — {(h.heure_fin || '').slice(0, 5)}</p>
            <p style={{ color: '#9CA3AF', fontSize: '13px' }}>{h.jours || 'Lun — Ven'}</p>
          </div>
        ))}
      </div>
      <div className={inView ? 'anim-fade-up d2' : ''} style={{ marginTop: '20px', padding: '16px 20px', borderRadius: '14px', background: 'linear-gradient(135deg,#FFF5F5,#FFECEC)', border: '1px solid rgba(212,25,26,0.18)', display: 'flex', gap: '12px', alignItems: 'center' }}>
...
/* ── Onglet vide avec placeholder ── */
function EmptyTab({ label, icon: Icon }) {
  return (
    <div style={{ textAlign: 'center', padding: '80px 20px' }}>
      <div style={{ width: '72px', height: '72px', borderRadius: '20px', backgroundColor: '#F2F9E5', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
        <Icon size={32} style={{ color: '#8DC31E' }} />
      </div>
      <h3 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: '18px', color: '#1F2937', marginBottom: '10px' }}>{label}</h3>
      <p style={{ color: '#9CA3AF', fontSize: '15px', maxWidth: '400px', margin: '0 auto 28px' }}>
        Cette section est en cours de mise à jour. Contactez-nous pour plus d'informations.
      </p>
      <Link to="/contacts" className="btn-green" style={{ display: 'inline-flex' }}>
  ...
  return (
    <div ref={ref} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {calendrier.map((ev, i) => {
        ...
        return (
          <div key={ev.id} className={inView ? `anim-fade-up d${Math.min(i + 1, 5)}` : ''} style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', background: '#ffffff', borderRadius: '16px', padding: '18px 20px', border: '1px solid #F3F4F6', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'box-shadow 0.2s,transform 0.2s' }}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                <span style={{ padding: '3px 10px', borderRadius: '50px', backgroundColor: col + '18', color: col, fontSize: '11px', fontWeight: 700 }}>{TYPE_LABELS[ev.type] || ev.type}</span>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <p style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, color: '#2D6A1F', fontSize: '14px' }}>{format(new Date(ev.date_debut), 'd MMM', { locale: fr })}</p>
              {ev.date_fin && ev.date_fin !== ev.date_debut && <p style={{ color: '#9CA3AF', fontSize: '12px', marginTop: '2px' }}>au {format(new Date(ev.date_fin), 'd MMM yyyy', { locale: fr })}</p>}
            </div>
          </div>
        )
      })}
...
/* ── Page principale ── */
export function InfosParentsPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Informations Parents — EPV MAREL | Abidjan</title>
        <meta name="description" content="Horaires, calendrier scolaire, inscription et informations pratiques pour les parents de l'EPV MAREL, Deux Plateaux Abidjan." />
      </Helmet>
      ...
      <section className="section" style={{ backgroundColor: '#F9FAFB' }}>
        <div className="wrap">
          ...
          {tab === 'inscription' && <EmptyTab label="Procédure d'inscription"    icon={FileText} />}
          {tab === 'scolarite'   && <EmptyTab label="Informations de scolarité"  icon={GraduationCap} />}
          {tab === 'cantine'     && <EmptyTab label="Service de cantine"         icon={Utensils} />}
          {tab === 'transport'   && <EmptyTab label="Service de transport"       icon={Bus} />}
          {tab === 'uniforme'    && <EmptyTab label="Tenue et uniforme"          icon={Shirt} />}
          {tab === 'calendrier'  && <CalendrierTab  calendrier={calendrier} />}
          {tab === 'reglement'   && <EmptyTab label="Règlement intérieur"        icon={ScrollText} />}
        </div>
      </section>
    </>
  )
...
export default InfosParentsPage